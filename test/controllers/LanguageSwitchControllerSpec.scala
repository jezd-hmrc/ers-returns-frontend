...
class LanguageSwitchControllerSpec extends UnitSpec  {
  "Hitting language selection endpoint" must {
    "redirect to Welsh translated start page if Welsh language is selected" in {
      ...
      header("Set-Cookie",result) shouldBe Some("PLAY_LANG=vy; Path=/;;PLAY_FLASH=switching-language=true; Path=/; HTTPOnly")
    }
