...
trait ErsConnector {
  ...
  def validateFileData(callbackData: UploadedSuccessfully, schemeInfo: SchemeInfo)(implicit authContext: AuthContext, request: Request[AnyRef], hc: HeaderCarrier): Future[HttpResponse] = {
    val empref: String = getAuthID(authContext).encodedValue
  ...
  def validateCsvFileData(callbackData: List[UploadedSuccessfully], schemeInfo: SchemeInfo)(implicit authContext: AuthContext, request: Request[AnyRef], hc: HeaderCarrier): Future[HttpResponse] = {
    val empref: String = getAuthID(authContext).encodedValue
